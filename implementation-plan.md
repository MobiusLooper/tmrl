...
* a fresh `tabular-local-v7` path with an eight-part, 583,200-state upper bound based on local lateral and heading context rather than absolute progress sectors
...
* versioned human-lap packages with deterministic state validation and learner-specific reward reconstruction
* fitted-Q demonstration pretraining for tabular runs and behavior cloning plus permanent demonstration replay for DQN
* bounded one-gate progress lookahead so valid manual laps, online training, and evaluation share terminal semantics
...
Implementation is complete. The DQN benchmark and a fresh Local-v7 acceptance run remain:
...
> A fresh seed-0 Local-v7 run must exceed the previous 77% canonical best progress within 3,000 episodes, then complete a canonical lap within 60 simulated seconds while averaging no more than two steering-state changes per second in its best replay.
...
Local-v5 enabled simultaneous brake-and-steer actions but learnt a conservative route that reached 77% by the 60-second limit and completed the lap only when replayed with a longer 74.95-second limit. Local-v6 kept those controls while making late checkpoint pace negative and doubling the timeout penalty to `-30`. Local-v7 adds bounded progress-gate lookahead and demonstration pretraining. Earlier local checkpoints remain replayable but cannot resume.
